...
echo route 192.168.6.0 255.255.255.0 >> /etc/openvpn/server.conf
echo  "# Tell all clients the Home address" >>/etc/openvpn/server.conf
echo push '"route 192.168.6.0 255.255.255.0"'  >> /etc/openvpn/server.conf
echo "# DNS-Server for Clients (Mango)">>/etc/openvpn/server.conf
echo push '"dhcp-option DNS 192.168.6.1"' >> /etc/openvpn/server.conf
